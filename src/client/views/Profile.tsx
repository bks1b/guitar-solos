...
import { StatsType, noSolos, request, resolveParams, updateParams } from '../util';
...
export default ({ name }: { name: string; }) => {
    ...
    useEffect(() => {
        request<Data>('profile/' + name, 'GET', undefined, () => setError(true)).then(setUser);
    }, []);
    useEffect(() => {
    ...
    useEffect(() => {
        updateParams([
            ...
            ...view
                ? [['view', 'stats'], ...statSortState.getParams()]
                : ratingSortState.getParams(),
        ]);
    });
    if (error) return <h1>User not found.</h1>;
    if (!user) return <></>;
    const arr = applyFilters(filterState.arr, user[2]);
    if (ratingSortState.sort) {
        const f = (x: Solos[number]) => [x[3], x[0].end - x[0].start, x[2].year][ratingSortState.sort - 1];
    ...
    return <>
        ...
        {user[2].length ? <>
            ...
            {view ? <RatingStats
                requestData={`profile/${name}/stats`}
                filterState={filterState}
                sortState={statSortState}
                sortDispatch={statSortDispatch}
                path={['profile', user[0]]}
                profile
            /> : <>
                <div style={{ marginBottom: 'var(--content-padding)' }}>
                    <Sort state={ratingSortState} dispatch={ratingSortDispatch}/>
                </div>
                {arr.length ? <List
                    length={arr.length}
                    step={STEP}
                    ...
                    render={c => <Albums
                        arr={arr.slice(0, c)}
                        navigateArtist={a => m => m
                            ? window.open(resolveParams(filterState.getParams(0, a)))
                            : filterDispatch(['filter', 0, [a], true])}
                        ratings ts
                    />}
                /> : noSolos}
            </>}
