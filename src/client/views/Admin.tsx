...
export default () => {
    ...
    return data
        ? <>
            ...
            {
                data.unverified[2].map((x, i) => <div key={i} {...navigateOnClick(['song', x[1].id])} className='link'>{getTimestamp(x[0].start)}-{getTimestamp(x[0].end)} <a className='label'>on</a> {x[1].name} <a className='label'>by</a> {x[2].artist}</div>)
            }
