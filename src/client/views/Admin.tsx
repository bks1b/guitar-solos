...
import { MainContext , getTimestamp, request } from '../util';
...
export default () => {
    ...
    useEffect(() => {
        ...
        request<Data>('admin/data').then(setData);
    }, []);
    return data ? <>
        <button onClick={() => request('admin/backup').then(d => {
        ...
        {data.unverified[2].map((x, i) => <div key={i} {
            ...navigateOnClick(['song', x[1].id], [['solo', x[0].id]])
        } className='link'>
        ...
        {(['guitarists', 'tags'] as const).map(k => data[k].length
            ? <div key={k}>
                <h1>Songs with solos without {k}</h1>
                <AdminSongs arr={data[k]}/>
            </div>
            : '')}
    </> : <></>;
};
...
} & Record<'guitarists' | 'tags', [Song, Album][]>;