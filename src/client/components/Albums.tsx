...
export default ({ arr, navigateArtist, ratings, album, ts }: { arr: Solos; navigateArtist?: (a: string) => [() => any, () => any]; ratings?: boolean; album?: boolean; ts?: boolean; }) => {
    ...
    return <div className='smallAlbums'>{arr.map((x, i) => <div key={i}>
        ...
        <h3 {...navigateOnClick([album ? 'album' : 'song', x[album ? 2 : 1].id])} className='link'>{x[album ? 2 : 1].name}</h3>
